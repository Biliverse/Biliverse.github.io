...
for (const name of names) {
  if (name === "Enhanced") continue; // Enhanced owns its PreferencePanes page and handler.
...
const enhancedAssets = path.join(repositories, "Enhanced/dist/settings");
for (const name of ["index.html", "app.mjs", "panel.css", "biliverse.css", "Enhanced.boxjs.json"])
  outputs.set(path.join(root, "docs/public/settings/assets", name), await readFile(path.join(enhancedAssets, name)));
const enhancedPage = await readFile(path.join(enhancedAssets, "index.html"));
