...
- 项目主页：把本站的 index.html、index.mjs 和图片素材映射到 app.bilibili.com；模块页使用的 `theme.css` 保留本站绝对地址，并可由 Enhanced 模板通过同版 `mock.js` 接管。官方 CSS 和 JSBridge SDK 始终直接使用官方地址。
...
预览读取同级业务仓库 `dist/config.dev.bundle.js`，以及 NSNanoCat/PreferencePanes 的 `dist/api.js` 和 `dist/web.js`，以独立内存模拟 Enhanced 提供通用前端与固定存储 API、各模块直接提供 BoxJS API 的代理拓扑。先构建这些仓库。预览不会复制 API、设置前端或配置到网站目录。
