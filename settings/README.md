...
本站维护 Biliverse 设置主页、一个与主页配套的页面脚本、可选项目主题和图片素材。Enhanced 唯一安装 PreferencePanes latest 的 `web.js`，为所有合法模块路径提供同一套设置前端；每个业务模块只安装自己的 `api.js` 规则并提供同版 BoxJS JSON。
...
- 设置前端：仅 Enhanced 用一条通用规则把任意合法 `/settings/{module}` 以及 `index.mjs`、`navigation.mjs` 交给 https://github.com/NSNanoCat/PreferencePanes/releases/latest/download/web.js。
- 模块 API：`/api/{module}` 及其 get/set/delete 动作引用 https://github.com/NSNanoCat/PreferencePanes/releases/latest/download/api.js。

Enhanced 的接入模板包含配置 Mock、唯一设置前端和 Enhanced 模块 API。Global、Redirect、ADBlock 的接入模板只包含各自的配置 Mock 与模块 API，不携带 `web.js` 或 `/settings/**` 规则。PreferencePanes 的 `api.js` 与 `web.js` 必须来自同一 Release，再更新业务模块模板。
...
预览读取同级业务仓库 dist/config.dev.bundle.js，以及 NSNanoCat/PreferencePanes 的 dist/api.js 和 dist/web.js，以独立内存模拟 Enhanced 提供前端、各模块提供配置与 API 的代理拓扑。先构建这些仓库。预览不会复制 API、设置前端或配置到网站目录。
